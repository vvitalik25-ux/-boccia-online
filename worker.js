...
export class BocciaRoom extends DurableObject {
  ...
  async fetch() {
    // Someone is connecting/reconnecting: keep the room alive.
  ...
  async webSocketMessage(ws, message) {
    ...
    if (data.type === "shot_result" && data.state) {
      if (data.state.phase === "finished") {
        // The match is over. Keep the live WebSockets/results screen,
        // but do not keep the finished game in persistent room storage.
  ...
  async scheduleCleanupIfEmpty() {
    // getWebSockets() is the source of truth even after hibernation.
  ...
  async alarm() {
    // If nobody returned during the 30-minute grace period,
    // remove all stored state/metadata for this room.
    if (this.ctx.getWebSockets().length === 0) {
    ...
    // A player came back before the alarm fired.
    await this.ctx.storage.deleteAlarm();
  }
